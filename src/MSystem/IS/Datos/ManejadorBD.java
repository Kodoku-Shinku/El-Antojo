...
public class ManejadorBD {
	...
	private ManejadorBD() throws DatabaseException {
		try {
			/*
			 * La instalacion del driver ocurre cuando se carga la clase en un
			 * entorno embebido esto inicia a Derby ya que no esta ejecutandose
			 */
			...
			/*
			 * En la conexion se especifica create=true para crear la base de
			 * datos la base de datos se crea bajo el directorio actual. Para
			 * borrarla, basta borrar el directorio que se creo.
			 */
			m_conexion = DriverManager.getConnection(protocol + m_nombreBD + ";create=true", props);

			// AutoCommit = true para que los cambios se repercutan
			// inmediatamente.
	 ...
	 *
	 * La primera vez que se invoca este metodo, se checa si ya se creo la
	 * instancia del Manejador de la Base de Datos. Si aun no ha sido creado, se
	 * instancia. Esto solo puede ocurrir una vez, por lo tanto esto representa
	 * el patron singleton.
	 *
	 * @return
	 */
	...
	/**
	 * El metodo finalize se sobre-escribe para cerrar la conexión y apagar la
	 * base de datos cuando el colector de basura recoge al manejador
	 *
	 */
